...
<div class="fieldcontain ${hasErrors(bean: physicalQuantityInstance, field: 'isoUnit', 'error')} ">
	<label for="isoUnit">
		<g:message code="physicalQuantity.isoUnit.label" default="Iso Unit" />
		
	</label>
	<g:select id="isoUnit" name="isoUnit.id" from="${cn.edu.cup.physical.QuantityUnit.findAllByPhysicalQuantity(physicalQuantityInstance)}" optionKey="id" value="${physicalQuantityInstance?.isoUnit?.id}" class="many-to-one" noSelection="['null': '']"/>

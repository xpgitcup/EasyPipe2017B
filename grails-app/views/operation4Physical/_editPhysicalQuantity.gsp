...
<html>
	...
	<body>
		...
		<div id="edit-physicalQuantity" class="content scaffold-edit" role="main">
			...
			<!--g:form url="[resource:physicalQuantityInstance, action:'update']" method="PUT" -->
			<g:form controller="operation4Physical" action="savePhysicalQuantity" id="${physicalQuantityInstance?.id}">
				<g:hiddenField name="version" value="${physicalQuantityInstance?.version}" />
				<fieldset class="form">
					<g:render template="formPhysicalQuantity"/>
				</fieldset>
				<fieldset class="buttons">
					<g:actionSubmit class="save" action="savePhysicalQuantity" value="${message(code: 'default.button.update.label', default: 'Update')}" />
				</fieldset>
			</g:form>
		</div>
	</body>
